import { electronicsProducts, womenFashionProducts } from './Allcatagory';
...
function Selling() {
  ...
  const length = electronicsProducts.length;
  const { addToCart, removeFromCart, addToWishlist, removeFromWishlist, wishlist, cart } = useCart(); 
  const navigate = useNavigate();

  const currentItems = electronicsProducts.slice(previous, farword);
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-16 py-10">
      ...
      <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-6">Best Selling Products</h1>
